...
export async function GET() {
  try {
    const raw = await getPageContent("industries")
    if (raw) {
      return NextResponse.json(normalizeIndustriesPageContent(raw as Record<string, unknown>))
    }
    return NextResponse.json(createDefaultIndustriesPageContent())
  } catch (error) {
    console.error("Get industries page content error:", error)
...
export async function PUT(request: NextRequest) {
  try {
    ...
    const normalized = normalizeIndustriesPageContent(body)

    await updatePageContent("industries", {
      pageKey: "industries",
      industriesPage: normalized,
      hero: {
    ...
    console.error("Update industries page content error:", error)
    return NextResponse.json({ error: "Internal server error" }, { status: 500 })
  }
}
