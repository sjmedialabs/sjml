...
import { sendRestaurantMarketingFormEmail } from "@/lib/email"
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Store in MongoDB
    try {
      const collection = await getCollection("restaurant_marketing_submissions")
      ...
      console.error("Failed to store restaurant submission in DB:", dbError)
      // Continue even if DB fails - still try to send email
    }

    // Send email
    try {
      await sendRestaurantMarketingFormEmail(formData)
    } catch (emailError) {
      console.error("Failed to send restaurant form email:", emailError)
      // Log the submission data so it's not lost
      console.log("RESTAURANT FORM SUBMISSION DATA (email failed):", JSON.stringify(formData, null, 2))
    }

    return NextResponse.json({ success: true }, { status: 200 })
  } catch (error) {
    console.error("Restaurant marketing form submission error:", error)
