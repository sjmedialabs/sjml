...
interface InsightDownloadModalProps {
  isOpen: boolean
  onClose: () => void
  pdfUrl: string
  insightTitle: string
...
export function InsightDownloadModal({ isOpen, onClose, pdfUrl, insightTitle }: InsightDownloadModalProps) {
  ...
  const handleVerifyOtp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (pdfUrl) {
        ...
        link.href = pdfUrl
        link.download = `${insightTitle.replace(/[^a-z0-9]/gi, "-").toLowerCase()}.pdf`
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative site-modal-panel rounded-2xl p-8 max-w-md w-full mx-4">
        ...
        <h2 className="text-2xl font-bold text-black mb-2">Download Insight</h2>
        <p className="text-black/60 mb-6">
          {step === "form" ? "Enter your details to receive this insight" : "Enter the OTP sent to your phone"}
        </p>
